...
echo [3/4] Upload to Vector...
type "%FLASH%" | %SSH% "mkdir -p /data/ota && cat > /data/unlock-manual-flash-v2.sh && chmod 755 /data/unlock-manual-flash-v2.sh"
if errorlevel 1 goto fail
type "%OTA%" | %SSH% "cat > /data/ota/v.ota"
if errorlevel 1 goto fail

echo [4/4] Flashing inactive slot (several minutes, then reboot)...
%SSH% "rm -f /data/unbrick; mount -o remount,rw /; sh /data/unlock-manual-flash-v2.sh /data/ota/v.ota"
if errorlevel 1 goto fail
