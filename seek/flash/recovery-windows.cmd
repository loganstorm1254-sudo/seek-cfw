...
rem -O = legacy scp (recovery has no sftp-server; Windows OpenSSH defaults to SFTP)
set SCP=scp -O -i "%KEY%" -o PubkeyAcceptedAlgorithms=+ssh-rsa -o HostKeyAlgorithms=+ssh-rsa -o ServerAliveInterval=15 -o ServerAliveCountMax=120
...
echo [3/4] Upload to Vector (recovery: /data/ota only, /ota is read-only)...
%SSH% "mkdir -p /data/ota && df -h /data /cache 2>/dev/null; rm -f /data/ota/v.ota"
%SCP% "%FLASH%" root@%IP%:/data/unlock-manual-flash-v2.sh
if errorlevel 1 goto fail
%SCP% "%OTA%" root@%IP%:%OTA_ON_ROBOT%
if errorlevel 1 goto fail
...
:fail
echo FAILED - check charger, WiFi, IP (%IP%), SSH key, and /data free space.
exit /b 1
