...
// Default Regular (0) — cooler for long sessions. Users can still pick Balanced/Performance in wired.
...
func (m *FreqChange) Description() string {
	return "Modifies CPU/RAM frequency. Regular is coolest for long runs; Performance can trigger the thermometer."
}
...
func (m *FreqChange) Load() error {
	...
	if err != nil {
		fmt.Println("(freq) Error loading contents, using cooler Regular preset (0)")
		freqInt = FreqPreset
		contents = FreqPresetStr
	}
	// Soft safety: if somehow invalid, fall back to Regular — never force Performance.
	if freqInt != 0 && freqInt != 1 && freqInt != 2 {
		freqInt = 0
		contents = "0"
	}
