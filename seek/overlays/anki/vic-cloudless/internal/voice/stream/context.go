...
func (strm *Streamer) bufferRoutine(streamSize int) {
	...
	flushRemaining := func() {
		for len(audioBuf) > 0 {
			...
			chunk := make([]byte, n)
			copy(chunk, audioBuf[:n])
			audioBuf = audioBuf[n:]
			select {
			case strm.audioStream <- chunk:
			case <-strm.ctx.Done():
				return
			}
		}
	...
	for {
		...
		case <-strm.micDone:
			// Mic AudioDone — flush leftover PCM so ASR can FinalResult.
			flushRemaining()
			return
		case <-strm.ctx.Done():
			return
...
func (strm *Streamer) addBytes(buf []byte) {
	// Never panic if the listen loop already finished (closed/canceled).
	...
	case <-strm.ctx.Done():
	case <-strm.micDone:
		// Mic done; drop trailing samples rather than block forever.
	}
}
