...
#define PROCEDURALFACE_SCANLINE_FEATURE      1 // Seek: Cozmo CRT scanlines (forced on when /data/seek/cozmo_mode)
...
public:
  ...
  // Seek Cozmo mode: live-polled from /data/seek/cozmo_mode (CCIS menu flag).
  // When active, procedural face uses CRT scanlines + smaller/flatter Cozmo-like eyes.
  static bool CozmoLookActive();
  static s32 GetDrawEyeWidth();
  static s32 GetDrawEyeHeight();
  static f32 GetCozmoScanlineOpacity(); // effective opacity when Cozmo look is on
