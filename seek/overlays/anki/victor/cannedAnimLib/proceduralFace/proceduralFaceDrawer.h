...
namespace Vector {
  ...
  private:
    ...
    // Cozmo CRT: every other row. Vector console scanlines: alternating pairs (00110011).
    static bool ShouldApplyScanlineToRow(const u32 rowNum) {
      return ProceduralFace::CozmoLookActive() ? ((rowNum & 1) != 0) : ((rowNum & 2) != 0);
    }
#endif
