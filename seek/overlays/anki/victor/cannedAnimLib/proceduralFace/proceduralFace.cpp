...
# undef CONSOLE_GROUP
  ...
  // Seek Cozmo look constants (visible to this TU via anonymous namespace).
  constexpr s32 kCozmoEyeSpacing = 70;   // V1 was 64; slightly wider for 184px face
  constexpr s32 kCozmoEyeWidth   = 32;   // V1: 30
  constexpr s32 kCozmoEyeHeight  = 42;   // V1: 40
  constexpr ProceduralFace::Value kCozmoScanlineOpacity = 0.52f;
...
bool ProceduralFace::CozmoLookActive()
{
  // Poll infrequently so toggling CCIS Cozmo mode takes effect without restarting vic-anim.
  static bool sActive = false;
  static double sLastCheckSec = 0.0;
  struct timespec now {};
  clock_gettime(CLOCK_MONOTONIC, &now);
  const double nowSec = static_cast<double>(now.tv_sec) + static_cast<double>(now.tv_nsec) * 1e-9;
  if ((nowSec - sLastCheckSec) > 0.5) {
    sLastCheckSec = nowSec;
    struct stat st {};
    sActive = (stat("/data/seek/cozmo_mode", &st) == 0) && S_ISREG(st.st_mode);
  }
  return sActive;
}

s32 ProceduralFace::GetDrawEyeWidth()
{
  return CozmoLookActive() ? kCozmoEyeWidth : NominalEyeWidth;
}

s32 ProceduralFace::GetDrawEyeHeight()
{
  return CozmoLookActive() ? kCozmoEyeHeight : NominalEyeHeight;
}

f32 ProceduralFace::GetCozmoScanlineOpacity()
{
  return kCozmoScanlineOpacity;
}
...
{
  const s32 spacing = CozmoLookActive() ? kCozmoEyeSpacing : kProcFace_NominalEyeSpacing;
...
{
  const s32 spacing = CozmoLookActive() ? kCozmoEyeSpacing : kProcFace_NominalEyeSpacing;
  return ProceduralFace::GetNominalLeftEyeX() + spacing;
}
