...
namespace Vector {
  ...
  // SeekOS: dashboard-written eye texture at this path.
  static const char* kSeekEyeOverlayPath = "/data/data/customFaceOverlay.jpg";
  static const char* kSeekEyeReloadFlag  = "/run/seek-eyes/reload";
  static time_t s_seekEyeMtime = 0;
  static bool   s_seekEyeActive = false;
...
void ProceduralFaceDrawer::LoadCustomEyePNG()
{
  ...
  s_seekEyeActive = false;
  s_seekEyeMtime = 0;

  auto loadImage = [&](const cv::String& path, f32 opacity) -> bool {
    ...
    for (int r = 0; r < img.rows; ++r) {
      ...
      for (int c = 0; c < img.cols; ++c) {
        u8 b, g, r8, a;
        if (hasAlpha) {
          b = src4[c][0]; g = src4[c][1]; r8 = src4[c][2]; a = src4[c][3];
        } else {
          b = src3[c][0]; g = src3[c][1]; r8 = src3[c][2];
          a = Util::Clamp<u8>(opacity * 255.f, 0, 255);
        }
        dstPix[c] = {
  ...
  // Seek dashboard upload wins when present (full opacity = image owns eye texture).
  struct stat st {};
  if (stat(kSeekEyeOverlayPath, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 0) {
    if (loadImage(kSeekEyeOverlayPath, 1.f)) {
      _hasCustomEyes = true;
      s_seekEyeActive = true;
      s_seekEyeMtime = st.st_mtime;
      return;
    }
  }
  ...
  if (!loadImage(kFaceOverlay, kProcFace_CustomEyeOpacity)) {
    return;
  }
  _hasCustomEyes = true;
}
...
bool ProceduralFaceDrawer::ApplyCustomOverlay(const ProceduralFace& faceData,
                                             ...
                                             bool dirty)
{
  // SeekOS: hot-reload when dashboard writes a new overlay (or clears it).
  {
    bool force = (access(kSeekEyeReloadFlag, F_OK) == 0);
    if (force) {
      unlink(kSeekEyeReloadFlag);
    }
    struct stat st {};
    const bool haveSeek = (stat(kSeekEyeOverlayPath, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 0);
    if (force || (haveSeek && (!s_seekEyeActive || st.st_mtime != s_seekEyeMtime)) ||
        (!haveSeek && s_seekEyeActive)) {
      LoadCustomEyePNG();
    }
  }

  const bool seekOrConsole = (s_seekEyeActive || kProcFace_CustomEyes) && _hasCustomEyes;
  if(seekOrConsole)
  {
    std::lock_guard<std::mutex> lk(gCustomEyeMtx);
    ...
    if (overlayW <= 0 || overlayH <= 0 || fullW <= 0 || fullH <= 0) {
      return dirty;
    }
