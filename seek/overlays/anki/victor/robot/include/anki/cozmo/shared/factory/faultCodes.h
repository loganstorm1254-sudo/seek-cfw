...
{
  // SeekOS: 898/899 are body-board (spine) comms failures. Never put them on
  // the face — vic-robot falls back to a dummy body so the LCD/eyes still boot.
  if (code == NO_BODY || code == SPINE_SELECT_TIMEOUT) {
    printf("DisplayFaultCode: ignoring body fault %u (SeekOS head-only)\n", code);
    return 0;
  }
