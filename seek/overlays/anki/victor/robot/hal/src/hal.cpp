...
namespace { // "Private members"
  ...
  // SeekOS head-only fallback: synthesized body so 898/899 never halt the face.
  BodyToHead dummyBodyData_{};
  bool dummyBodyMode_ = false;
  bool spineOpened_ = false;
...
BodyToHead BootBodyData_{
  ...
  void sanitize_live_body(BodyToHead* b)
  {
    ...
    if (b->battery.charger < kMinChargerContactCounts) {
      ...
      b->battery.charger = 0;
    } else if ((b->battery.flags & POWER_IS_CHARGING) &&
               !(b->battery.flags & POWER_ON_CHARGER)) {
      // Inconsistent syscon frame: charging without contacts.
      b->battery.flags = static_cast<BatteryFlags>(
          b->battery.flags & ~POWER_IS_CHARGING);
    }
  }
  ...
  static const char* kForceHeadOnlyPath = "/data/seek/head_only";

  void fill_dummy_body_data()
  {
    memset(&dummyBodyData_, 0, sizeof(dummyBodyData_));
    ...
    // Healthy pack, not on charger, so idle-face/eyes still run.
    dummyBodyData_.battery.main_voltage = static_cast<int16_t>(3.9f / kBatteryScale);
    dummyBodyData_.battery.charger = 0;
    dummyBodyData_.battery.temperature = 25;
    dummyBodyData_.battery.flags = static_cast<BatteryFlags>(0);
    dummyBodyData_.proximity.rangeMM = 120;
  }
  ...
  bool should_force_head_only()
  {
#ifdef HAL_DUMMY_BODY
    return true;
#else
    return access(kForceHeadOnlyPath, F_OK) == 0;
#endif
  }
  ...
  void enter_dummy_body_mode(const char* reason)
  {
    ...
    AnkiWarn("HAL.DummyBody.Enable", "%s", reason ? reason : "unknown");
...
Result HAL::Init(const int * shutdownSignal)
{
...
#ifndef HAL_DUMMY_BODY
  {
    ...
    if (errCode != err_OK) {
      enter_dummy_body_mode("spine_open failed (SeekOS: skip 899)");
    } else {
      spineOpened_ = true;
...
#else
  enter_dummy_body_mode("HAL_DUMMY_BODY");
#endif
