...
namespace {
  constexpr const char * kSupportURL = "";
...
void DrawFaultCode(uint16_t fault, bool willRestart)
{
  ...
  // the bottom of the screen (no Wire/Seek portal URL)
...
bool DrawStartPairingScreen(const std::string& robotName)
{
  ...
  {
    std::string shown = robotName;
    size_t pos = 0;
    while ((pos = shown.find("Vector", pos)) != std::string::npos) { shown.replace(pos, 6, "Victor"); pos += 6; }
    pos = 0;
    while ((pos = shown.find("vector", pos)) != std::string::npos) { shown.replace(pos, 6, "victor"); pos += 6; }
    img.DrawTextCenteredHorizontally(shown, kNormalFont, kRobotNameScale, kTextThickness, kWhiteColor, kRobotNameVerticalPosition, kDrawTwice);
  }
