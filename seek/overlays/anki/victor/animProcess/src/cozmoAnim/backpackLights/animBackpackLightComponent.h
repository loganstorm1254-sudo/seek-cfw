...
public:
  ...
  // Live spine sense from RobotState — clears stuck charging lights when off dock
  // even if BatteryStatus lags with stale IS_CHARGING / IS_ON_CHARGER bits.
  void UpdateChargerSense(float chargerVoltage, float batteryVoltage, uint32_t robotStatus);
