...
func (ctm *ClientTokenManager) Init() error {
	...
	if err != nil {
		...
		token.PerRuntimeToken = strings.TrimSpace(string(f))
	}
...
func (ctm *ClientTokenManager) CheckToken(clientToken string) (string, error) {
	...
	// Accept wired's per-runtime token even when no phone-app session tokens exist.
	if token.PerRuntimeToken != "" && strings.TrimSpace(token.PerRuntimeToken) == clientToken {
		return "wired", nil
	}
