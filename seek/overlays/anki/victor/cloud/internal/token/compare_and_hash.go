...
func CompareHashAndToken(hashedToken, token string) error {
	// Wired / on-robot tools use perRuntimeToken (not app session hashes).
	// Must check before base64 decode — the runtime token is plain alphanumeric.
	token = strings.TrimSpace(token)
	if PerRuntimeToken != "" && strings.TrimSpace(PerRuntimeToken) == token {
		return nil
	}
