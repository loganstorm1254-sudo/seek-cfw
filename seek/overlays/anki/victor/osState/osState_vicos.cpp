...
const std::string& OSState::GetOSBuildVersion()
{
  ...
  {
    // Prefer Seek/Anki OTA prop so double-click CCIS Main shows e.g. 3.0.1.9d
    // (ro.build.display.id is often empty or a long Yocto string on CFW).
    _osBuildVersion = GetProperty("ro.anki.version");
    if(_osBuildVersion.empty()) {
      _osBuildVersion = GetProperty("ro.build.display.id");
    }
    if(_osBuildVersion.empty() && Util::FileUtils::FileExists("/etc/os-version")) {
      _osBuildVersion = Util::FileUtils::ReadFile("/etc/os-version");
      Anki::Util::StringTrimWhitespaceFromEnd(_osBuildVersion);
    }
  }
