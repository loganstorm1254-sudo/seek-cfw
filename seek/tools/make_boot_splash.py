#!/usr/bin/env python3
"""Convert the DPRK passport cover into Vector RGB565 boot assets.

Vector's face is 184x96 (Santek / 1.0) or 160x80 (Midas / 2.0), RGB565 LE.
Rampost's anki_dev_unit.h is a C array of those 184x96 pixels.
vic-bootAnim loops boot_anim.raw (concatenated RGB565 frames).
"""
...
def main() -> None:
    ...
    overlay_anims = repo / "seek" / "overlays" / "anki" / "victor" / "resources" / "config" / "engine" / "animations"
    ...
    write_c_header(
        ...
        "DPRK passport cover static boot splash (184x96 RGB565). "
        "Top text: 조선민주주의인민공화국 / DEMOCRATIC PEOPLE'S REPUBLIC OF KOREA",
    )

    # Single-frame boot anim: vic-bootAnim loops until stopped, so a static
    # image never animates and never shows 890/899 during this phase.
    ...
    print(f"wrote boot_anim.raw frames=1 bytes={len(raw)}")
    print(f"wrote boot_anim_20.raw bytes={len(raw_v2)}")
