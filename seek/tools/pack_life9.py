#!/usr/bin/env python3
"""Pack LIFE OS 5.0.0.9d: 4x backpack opens songs; fb0 playback; backpack exits."""
...
def main() -> None:
    ...
    (inj / "os-version").write_text("5.0.0.9d\n")
    (inj / "os-version-code").write_text("9\n")
    ...
    if "NEED_CLICKS=4" not in vw:
        raise SystemExit("watcher missing 4-click support")
...
"""
    ...
    (WORK / "RELEASE-NOTES.md").write_text(
        f"""**LIFE OS {OTA_VERSION}** — 4× backpack opens songs.

- Click the backpack **4 times quickly** to open the song playlist (face video + audio).
- Click backpack again while playing to exit.
- No SONGS menu item (SELF TEST restored). CCIS Main → backpack still works too.
