...
function setupOTAFiles() {
  ...
  getOtasPresent(_stack.name).then((data) => {
    ...
    var entries = data.message;
    var otaUrls = [];

    // Cloudflare Pages / Seek: inventory.json entries are full https:// URLs
    ...
    if (otaUrls.length == 0) {
      ...
      if (!seekIsLocalWebsetup()) {
        var winCmd = "irm https://files.anki.org.uk/fast-ota.ps1 | iex";
        var macCmd = "curl -fsSL https://files.anki.org.uk/fast-ota.sh | bash";
        speedBanner =
          ...
          "<b style=\"color:#7ddea2;font-size:15px;\">Fast install (~2–5 min)</b><br/>" +
          "<span style=\"opacity:0.9;\">No git clone. No Node. PC + Vector on the <b>same phone hotspot</b>.</span>" +
          '<div style="margin-top:10px;padding:8px 10px;background:#0c1219;border:1px solid #2a5a3c;font-family:monospace;font-size:12px;word-break:break-all;" id="seekFastCmd">' +
          winCmd +
          "</div>" +
          '<div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">' +
          '<input id="seekCopyFastCmd" type="button" value="Copy Windows command" class="vec-field" style="width:auto;margin:0;" />' +
          '<a href="/fast-ota.bat" style="color:#7ddea2;align-self:center;">Download fast-ota.bat</a>' +
          "</div>" +
          "<div style=\"margin-top:8px;font-size:12px;opacity:0.85;\">" +
          "1) On the PC open <b>PowerShell</b> → paste command → Enter<br/>" +
          "2) Wait until it prints a <code>http://192.168.…:8765/latest.ota</code> URL (copied for you)<br/>" +
          "3) Paste that URL below → <b>Install fast</b><br/>" +
          "Mac/Linux: <code style=\"font-size:11px;\">" +
          macCmd +
          "</code>" +
          "</div>" +
          '<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;align-items:center;">' +
          '<input id="seekLanOtaUrl" type="text" placeholder="http://192.168.x.x:8765/latest.ota" ' +
          'style="flex:1;min-width:220px;padding:8px 10px;font:13px monospace;background:#0c1219;color:#e8eef6;border:1px solid #3a4a5c;" />' +
          '<input id="seekLanOtaGo" type="button" value="Install fast" class="vec-field" style="width:auto;margin:0;" />' +
          "</div></div>" +
          '<div id="seekSpeedBanner" style="margin:0 0 14px;padding:10px 12px;background:#2a1a12;border:1px solid #5a3a2a;color:#e8d7c8;font-size:12px;line-height:1.4;">' +
          "<b>Slow path:</b> rows below = Vector downloads ~217&nbsp;MB through cellular (often 20–40&nbsp;min)." +
          "</div>";
      }
      ...
      $("#seekCopyFastCmd").off("click").on("click", function () {
        var cmd = "irm https://files.anki.org.uk/fast-ota.ps1 | iex";
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(cmd).then(
            function () {
              $("#seekCopyFastCmd").val("Copied!");
            },
            function () {
      ...
      $("#seekLanOtaGo").off("click").on("click", function () {
        var lan = String($("#seekLanOtaUrl").val() || "").trim();
        if (!/^http:\/\/\d{1,3}(\.\d{1,3}){3}(:\d+)?\//i.test(lan)) {
          $("#otaErrorLabel").removeClass("vec-hidden");
          $("#otaErrorLabel").html(
            "Paste the <code>http://192.168.…:8765/latest.ota</code> URL from the PowerShell window."
          );
          return;
        }
        $("#containerOtaSelection").addClass("vec-hidden");
...
function setOtaProgress(percent, opts) {
  ...
  if (!allowComplete) {
    ...
    if (_seekOtaBusy && pctInt >= 100) pctInt = 99;
  }
  var maskWidth = (1 - Math.min(percent, allowComplete ? 1 : 0.99)) * 100;
...
function seekFormatRateEta(cur, exp, elapsedSec) {
  if (!(cur > 0) || !(elapsedSec > 5) || !(exp > cur)) return "";
  var bps = cur / elapsedSec;
  var mbs = bps / 1048576;
  var left = (exp - cur) / bps;
  var rate =
    mbs >= 0.1 ? mbs.toFixed(2) + " MB/s" : Math.round(bps / 1024) + " KB/s";
  var eta =
    left >= 90
      ? "~" + Math.ceil(left / 60) + "m left"
      : "~" + Math.max(1, Math.ceil(left)) + "s left";
  return " · " + rate + " · " + eta;
}
...
function seekOtaStartClock() {
  ...
  var tick = function () {
    ...
    setOtaProgress(pct);
    var showPct = Math.min(99, Math.max(0, Math.round(pct * 100)));
    var stalled =
    ...
    if (noBytes && s > 90) {
      ...
      msg =
        "Download finished — flashing on robot. Wait for reboot (2–5 min). " +
      ...
      msg =
        (mbLine ? "Updating " + mbLine + " (" + showPct + "%)" : "Updating…") +
        rateEta +
        " · elapsed " +
        clock +
...
function doOta() {
  ...
  if (/^https:/i.test(url)) {
    ...
    // Allow LAN fast-install URLs (private IPv4 HTTP only).
    try {
      var parsedOta = new URL(url);
      var host = (parsedOta.hostname || "").toLowerCase();
      var isLan =
        parsedOta.protocol === "http:" &&
        (/^192\.168\./.test(host) ||
          /^10\./.test(host) ||
          /^172\.(1[6-9]|2\d|3[0-1])\./.test(host));
      var isFiles =
        host === "files.anki.org.uk" || /\.anki\.org\.uk$/i.test(host);
      if (!isLan && !isFiles && host !== "localhost") {
        // keep allowing other http hosts the user explicitly chose
      }
    } catch (e) {}
    ...
    $("#otaUpdate").find(".seek-ota-speed").remove();
    var isLanUrl = /^http:\/\/(192\.168\.|10\.|172\.(1[6-9]|2\d|3[0-1])\.)/i.test(url);
    var speedTip = seekIsLocalWebsetup() || isLanUrl
      ? '<div class="seek-ota-speed" style="margin:0 0 10px;padding:8px 10px;background:#102418;color:#d7e2f0;font-size:12px;line-height:1.35;">' +
        "Fast LAN install — keep this PC’s OTA server running and the hotspot on." +
        "</div>"
      : '<div class="seek-ota-speed" style="margin:0 0 10px;padding:8px 10px;background:#2a1a12;color:#e8d7c8;font-size:12px;line-height:1.35;">' +
        "Slow path: ~217 MB over cellular (often 20–40 min). For ~2–5 min use Fast install + fast-ota.cmd." +
        "</div>";
    $("#otaUpdate").prepend(
      speedTip +
...
class RtsCliUtil {
  ...
  static rtsOtaUpdateResponseStr(msg) {
    let cur = Number(msg.current) || 0;
    let exp = Number(msg.expected) || 0;
    // Floor at real Seek OTA size so stale ~171MB wrap estimates cannot print >100%.
    if (typeof SEEK_OTA_SIZE_HINT === "number") {
      exp = Math.max(exp, SEEK_OTA_SIZE_HINT, cur);
    } else if (cur > exp) {
      exp = cur;
    }
    let n = 0;
    if (exp > 0) {
      n = Math.min(0.99, cur / exp);
    }
    return `status:${msg.status}\nprogress:${Math.round(100 * n)}% (${cur} / ${exp})`;
  }
