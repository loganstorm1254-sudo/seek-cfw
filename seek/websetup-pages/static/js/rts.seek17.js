    ...
    if (otaUrls.length == 0) {
      ...
      if (!seekIsLocalWebsetup()) {
        speedBanner =
          ...
          "<b style=\"color:#7ddea2;\">Fast install (~2–5 min)</b> — Vector must NOT download 217&nbsp;MB over cellular.<br/>" +
          "1. On this PC join the <b>same phone hotspot</b> as Vector<br/>" +
          "2. Run <code>seek\\websetup\\fast-ota.cmd</code> (or <code>node bin/seek-web-setup.js fast-ota</code>)<br/>" +
          "3. Paste the <code>http://192.168.…:8765/latest.ota</code> URL it prints, then Install<br/>" +
          ...
          '<input id="seekLanOtaGo" type="button" value="Install fast (LAN)" class="vec-field" style="width:auto;margin:0;" />' +
          "</div></div>" +
          ...
          "<b>Slow path:</b> Cloudflare rows below pull ~217&nbsp;MB through the phone’s cellular link (often 20–40&nbsp;min). Prefer Fast install above." +
          "</div>";
      }
      ...
      $("#seekLanOtaGo").off("click").on("click", function () {
        ...
        if (!/^http:\/\/\d{1,3}(\.\d{1,3}){3}(:\d+)?\//i.test(lan)) {
          ...
          $("#otaErrorLabel").html(
            "Fast install needs a plain HTTP LAN URL like " +
              "<code>http://192.168.43.10:8765/latest.ota</code> " +
              "(from <code>fast-ota.cmd</code> on this PC)."
          );
          return;
        }
        $("#containerOtaSelection").addClass("vec-hidden");
