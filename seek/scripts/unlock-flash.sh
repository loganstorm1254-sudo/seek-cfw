#!/bin/sh
# Unlock manual flash — stream from /ota/v.ota (no full extract; /ota is tiny).
# Unlock's Python update-engine is a stub. Stay on charger.
#
...
if [ "$BST" -lt 1000000 ]; then
  ...
  ls -la "$PAS"; xxd "$PAS" 2>/dev/null | head -2 || od -Ax -tx1 "$PAS" | head -2
  exit 1
fi
...
if dd --help 2>&1 | grep -q status; then
  eval "tar -xOf '$OTA' apq8009-robot-sysfs.img.gz | $PIPE | dd of='$SYS_DEV' bs=1M status=progress conv=fsync"
else
  eval "tar -xOf '$OTA' apq8009-robot-sysfs.img.gz | $PIPE | dd of='$SYS_DEV' bs=1M conv=fsync"
fi
sync
